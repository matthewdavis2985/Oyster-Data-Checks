...
ERR_Table1 <- rbind(ERR_Table1a, ERR_Table1b, ERR_Table1c, ERR_Table1d, ERR_Table1e, ERR_Tablex) %>%
  arrange(schema, Table, PrimaryKey)
...
ERR_Table2 <- distinct(ERR_Table2) %>%
  arrange(Table, OrphanForeignKey) 
...
ERR_Table3 <- distinct(ERR_Table3) %>%
  arrange(Table, ChildlessPrimaryKey) 
...
ShellBudgetQuad_range4 <- filter(ShellBudgetQuadrat1, 
  ...
  rename(TotalWt = TotalSampleWeight, TotalVol = TotalSampleVolume, LiveOyWt = LiveOysterWeight, LiveOyVol = LiveOysterVolume) %>%
  select(QuadratID, TotalWt, TotalVol, LiveOyWt, LiveOyVol, Comments) #Relative ranges on an individual basis, top of the sheet

ShellBudgetQuad_range5a <- filter(ShellBudgetQuadrat1, 
                                  OysterShellVolume != 0.01 &
  ...
  select(QuadratID, OyShellWt, OyShellVol, Comments) #Relative ranges on an individual basis, bottom of the sheet

ShellBudgetQuad_range5b <- filter(ShellBudgetQuadrat1,
                                  PlantedShellVolume != 0.01 &
  ...
  select(QuadratID, PlantedWt, PlantedVol, Comments) #Relative ranges on an individual basis, bottom of the sheet

ShellBudgetQuad_range5c <- filter(ShellBudgetQuadrat1, 
                                  ShellHashVolume != 0.01 &
  ...
  select(QuadratID, HashWt, HashVol, Comments) #Relative ranges on an individual basis, bottom of the sheet

ShellBudgetQuad_range5d <- filter(ShellBudgetQuadrat1,
                                  BlackAndOtherSubstrateVolume != 0.01 &
  ...
  select(QuadratID, BlkWt, BlkVol, Comments) #Relative ranges on an individual basis, bottom of the sheet
...
if (nrow(ShellBudgetQuad_range1a) > 0 | nrow(ShellBudgetQuad_range1b) > 0 | nrow(ShellBudgetQuad_range2) > 0 | nrow(ShellBudgetQuad_range3) > 0 | nrow(ShellBudgetQuad_range4) > 0 | nrow(ShellBudgetQuad_range5a) > 0 | nrow(ShellBudgetQuad_range5b) > 0 | nrow(ShellBudgetQuad_range5c) > 0 | nrow(ShellBudgetQuad_range5d) > 0) {
