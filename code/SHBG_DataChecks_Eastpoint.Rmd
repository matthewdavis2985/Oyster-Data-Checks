...
initialSHmeasurement <- filter(ShellBudgetSH1, substring(ShellHeightID, 29, 31) == "001") %>%
  rename("InitialSH" = "ShellHeight") # Pulls in the initial SH for each quadrat

SHmeasures <- as.data.frame(table(ShellBudgetSH1$QuadratID)) %>% # Counts up the number of SH measurements that were made
...
initialSHmeasurement <- filter(ShellBudgetSH1, substring(ShellHeightID, 29, 31) == "051") %>%
...
TooManyTooFew <- SHmeasures %>%
  filter(NumLive != NumMeasured) %>%
  filter(!(NumLive > 50 & NumMeasured == 50)) %>%
  filter(!(is.na(InitialSH) & NumDead == 0 & NumMeasured == 1)) %>%
  mutate(ErrorColumn = "Too many or too few measured")
...
`r if (nrow(null_ShellBudgetSHLive) > 0) {kable(null_ShellBudgetSHLive, caption = "Null Shell Budget Shell Height Data. Null values should only be recorded when no oysters were measured. ShellHeightID should end _001 for Live.")}`

`r if (nrow(null_ShellBudgetSHDead) > 0) {kable(null_ShellBudgetSHDead, caption = "Null Shell Budget Shell Height Data. Null values should only be recorded when no oysters were measured. ShellHeightID should end _051 for Dead.")}`
