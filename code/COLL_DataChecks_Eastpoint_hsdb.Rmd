...
# Set your variables
CheckStart <- as.Date("2021-07-01")  # Start Date for Checks
...
Database = "OysterLocalMD20240216"
...
null_SampleEvents <- filter(SampleEvent1, is.na(LatitudeDec) | is.na(LongitudeDec)) %>%
  filter(Comments != "Station not visited") %>%
  select(SampleEventID, LatitudeDec, LongitudeDec, Comments)
...
null_WQs <- filter(SampleEventWQ1, is.na(Temperature) | is.na(Salinity) | is.na(DissolvedOxygen) | is.na(pH) | is.na(Depth)) %>%
  filter(!str_detect(Comments, "Station not visited")) %>%
...
EXECUTE [hsdb].[spChecksCollections]
