...
# Set your variables
CheckStart <- as.Date("2020-01-01")  # Start Date for Checks
CheckEnd <- as.Date("2020-08-28")  # End Date for Checks
...
if (nrow(Repro1[duplicated(Repro1$OysterID), ]) > 0) {
  ERR_Table1f <- bind_rows(ERR_Table1, data.frame(schema = "hsdb", Table = "Repro", ReproSH1[duplicated(Repro1$OysterID), ])) %>% 
    mutate(PrimaryKey = OysterID) %>%
    select(schema, Table, PrimaryKey)
}
...
null_SampleEvents <- filter(SampleEvent1, is.na(LatitudeDec) | is.na(LongitudeDec)) %>%
  select(SampleEventID, LatitudeDec, LongitudeDec)
