...
Database = "OysterLocalMD20230804"
```
...
rm(dest, FixedLocations1, hsdbRecruitment, hsdbSampleEvent, hsdbSampleEventWQ, hsdbTripInfo, fkid, pkid, numb, sorc, tabl, check_id, i, j)
...
null_WQs <- filter(SampleEventWQ1, is.na(Temperature) | is.na(Salinity) | is.na(DissolvedOxygen) | is.na(Depth)) %>%
  select(SampleEventWQID, StationNumber, SampleDepth, Depth, Temperature, Salinity, DissolvedOxygen, Comments)
...
EXEC sp_CompleteRecruitment_hsdb \'`r EstuaryCode`\', \'`r CheckStart`\',
...
UPDATE hsdb.Recruitment SET DataStatus = \'Completed\', CompletedBy =
\'`r DataManager`\', DateCompleted = \'`r format(Sys.Date(), "%Y-%m-%d")`\'
WHERE SampleEventID IN (SELECT SampleEventID FROM SampleEvent WHERE
TripID IN (SELECT TripID FROM #ValidTrips));
