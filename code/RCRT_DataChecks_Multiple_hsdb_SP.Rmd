...
# Set your variables
CheckStart <- as.Date("2008-01-01")  # Start Date for Checks
CheckEnd <- as.Date("2008-12-30")  # End Date for Checks
...
TripInfo1 <- hsdbTripInfo %>% 
  filter(substring(TripID,1,2) %in% EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & TripType == "Recruitment") %>%
  arrange(TripID)
...
SampleEvent1 <- hsdbSampleEvent %>% 
  ...
  filter(substring(SampleEventID,1,2) %in% EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(SampleEventID,3,6) == "RCRT" & DataStatus == "Proofed") %>%
...
SampleEventWQ1 <- hsdbSampleEventWQ %>% 
  ...
  filter(substring(SampleEventWQID,1,2) %in% EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(SampleEventWQID,3,6) == "RCRT" & DataStatus == "Proofed") %>%
  left_join(FixedLocations1, SampleEventWQ1, by = c("FixedLocationID")) %>%
  arrange(TripDate, StationNumber, SampleEventWQID)

Recruitment1 <- hsdbRecruitment %>% 
  mutate(TripDate = as.Date(substring(SampleEventID, 8, 15), format = "%Y%m%d"),
  ...
  filter(case_when(Estuary == "TB" ~ ShellPosition < 7, T ~ ShellPosition < 15))

Rcrt_dates <- Recruitment1 %>% dplyr::select(SampleEventID, Estuary, StationNumber, DeployedDate, NumDays, Comments) %>% unique() %>% arrange(SampleEventID)
...
if (nrow(Recruitment1[duplicated(Recruitment1$ShellID), ]) > 0) {
  ERR_Table1d <- bind_rows(ERR_Table1, data.frame(schema = "hsdb", Table = "Recruitment", Recruitment1[duplicated(Recruitment1$ShellID), ])) %>% 
...
ERR_Table3 <- distinct(ERR_Table3) %>% arrange(check_id)
...
Recruitment_nulls <- filter(Recruitment1, is.na(ShellID) | is.na(SampleEventID) | is.na(DeployedDate) | is.na(ShellReplicate) | is.na(ShellPosition)) %>%
  select(ShellID, SampleEventID, DeployedDate, ShellReplicate, ShellPosition, Comments)

# Remove unneeded data frames and values

rm(dboFixedLocations, dest, FixedLocations1, hsdbRecruitment, hsdbSampleEvent, hsdbSampleEventWQ, hsdbTripInfo, fkid, pkid, numb, sorc, tabl, check_id, i, j)
...
null_WQs <- filter(SampleEventWQ1, is.na(Temperature) | is.na(Salinity) | is.na(DissolvedOxygen) | is.na(pH) | is.na(Depth) | is.na(SampleDepth) | is.na(PercentDissolvedOxygen) | is.na(TurbidityYSI) | is.na(TurbidityHach)) %>% filter(!grepl(paste(c("ame as", "ame WQ", "ame Water", "ame water"), collapse = '|'), Comments)) %>%
  select(SampleEventWQID, StationNumber, SampleDepth, Depth, Temperature, Salinity, DissolvedOxygen, PercentDissolvedOxygen, pH, TurbidityYSI, TurbidityHach, Comments)
...
DO_range <- SampleEventWQ1 %>% 
  ...
  select(SampleEventWQID, StationNumber, SampleDepth, DissolvedOxygen, DO.Sat, PercentDissolvedOxygen, Comments)
...
null_Recs <- if("CR" %in% EstuaryCode) {rbind(filter(Recruitment1, Estuary == "CR" & ShellPosition >= 2 & ShellPosition <= 11) %>% filter(is.na(NumBottom) & is.na(Comments)) %>% select(TripDate, DeployedDate, StationNumber, FixedLocationID, ShellReplicate, ShellPosition, NumBottom, Comments),
                                              filter(Recruitment1, Estuary != "CR" & ShellPosition >= 2 & ShellPosition <= 5 | ShellPosition >= 8 & ShellPosition <= 11) %>% 
  filter(is.na(NumBottom) & is.na(Comments)) %>%
  select(TripDate, DeployedDate, StationNumber, FixedLocationID, ShellReplicate, ShellPosition, NumBottom, Comments))
  } else {
    filter(Recruitment1, ShellPosition >= 2 & ShellPosition <= 5 | ShellPosition >= 8 & ShellPosition <= 11) %>% 
  filter(is.na(NumBottom) & is.na(Comments)) %>%
  select(TripDate, DeployedDate, StationNumber, FixedLocationID, ShellReplicate, ShellPosition, NumBottom, Comments)}

  # Out of range values
Days_range <- filter(Recruitment1, NumDays < 21 | NumDays > 36) %>%
  select(SampleEventID, StationNumber, TripDate, DeployedDate, NumDays, StationNumber, ShellReplicate) %>%
  distinct()
...
Spat_range <- filter(Recruitment1, NumBottom > 200) %>%
...
for(i in list(EstuaryCode)) {
  checks <- NULL
  checks[i] <- paste0("EXECUTE [dbo].[spChecksRecruitment] @CheckStart = &apos;", CheckStart, "&apos;, @CheckEnd = &apos;", CheckEnd, "&apos;, @EstuaryCode = &apos;", i, "&apos;, @DataManager = &apos;", DataManager, "&apos;;")
  }
```
...
`r kable(Rcrt_dates, caption = "Trips included have the following deployment dates.")`
...
`r if (nrow(TurbidityHach_range) > 0) {kable(TurbidityYSI_range, caption = "Out of range Turbidity data. Expected range is -2.0 to 30 NTU. Comment required for out of range values.")}`
