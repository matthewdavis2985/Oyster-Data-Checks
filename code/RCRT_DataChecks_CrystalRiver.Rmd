...
EstuaryCode = "SS"  # Estuary Code. Use 2-letter code in Primary Keys
DataManager = "Tomena Scholze"
Database = "OystersLocal20240605"

Server = "localhost\\TOMENALOCALSQL"
```
...
null_Recs <- if(EstuaryCode == "CR") {filter(Recruitment1, ShellPosition >= 2 & ShellPosition <= 11) %>% filter(is.na(NumBottom) & is.na(Comments)) %>% select(TripDate, DeployedDate, StationNumber, FixedLocationID, ShellReplicate, ShellPosition, NumBottom, Comments)
  } else {
