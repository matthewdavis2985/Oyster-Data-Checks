...
output:
  html_document:
params:
    Sites: ["CR"] #CR must be run seperately due to CERP & TIG sampling
---
...
EstuaryCode = params$Sites  #params$Sites for multiple or use 2-letter code in Primary Keys for single
...
hsdbTripInfo <- hsdbTripInfo %>% 
  filter(substring(TripID,1,2) %in% EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(TripID,3,6) == "SRVY")  %>%
  arrange(TripID)

hsdbSampleEvent <- hsdbSampleEvent %>% 
  mutate(TripDate = as.Date(substring(SampleEventID, 8, 15), format = "%Y%m%d")) %>%
  filter(substring(SampleEventID,1,2) %in% EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(SampleEventID,3,6) == "SRVY") %>%
  arrange(TripDate)

hsdbSampleEventWQ <- hsdbSampleEventWQ %>% 
  mutate(TripDate = as.Date(substring(SampleEventWQID, 8, 15), format = "%Y%m%d")) %>%
  filter(substring(SampleEventWQID,1,2) %in% EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(SampleEventWQID,3,6) == "SRVY") %>%
  arrange(TripDate)
...
SurveySH_nulls <- filter(SurveySH1, is.na(ShellHeightID) | is.na(QuadratID) | is.na(EnteredBy) | is.na(DateEntered) | is.na(ProofedBy) | is.na(DateProofed) ) %>%
...
null_SurveyQuads <- if(EstuaryCode %in% "SL" | EstuaryCode %in% "LX" | EstuaryCode %in% "LW") {
  ...
  # Out of range values
SurveyQuad_range <- if(EstuaryCode %in% "SL" | EstuaryCode %in% "LX" | EstuaryCode %in% "LW") {
...
if(EstuaryCode %in% "SL" | EstuaryCode %in% "LX" | EstuaryCode %in% "LW"){
  Over10Measured <- SHmeasures %>% filter(NumMeasured > 10) %>% mutate(ErrorColumn = "Over 10 measured")
} else if(EstuaryCode %in% "CR"){
  Over10Measured <- SHmeasures %>% filter(QuadratNumber < 11 & NumMeasured > 10) %>% mutate(ErrorColumn = "Over 10 measured")
  Over50Measured <- SHmeasures %>% filter(QuadratNumber > 10 & NumMeasured > 50) %>% mutate(ErrorColumn = "Over 50 measured")
}else {
...
if(EstuaryCode %in% "SL" | EstuaryCode %in% "LX" | EstuaryCode %in% "LW"){
  ...
  TooManyTooFew <- SHmeasures %>% filter(NumLive != NumMeasured) %>% mutate(Location = as.numeric(substring(QuadratID, 19, 22))) %>%
    filter((!(NumLive > 10 & NumMeasured == 10) & QuadratNumber < 11 & !is.na(Location)) | (!(NumLive > 50 & NumMeasured == 50) & QuadratNumber > 10)) %>% 
    filter(!(is.na(InitialSH) & NumLive == 0 & NumMeasured == 1)) %>% dplyr::select(-Location) %>%
    mutate(ErrorColumn = "Too many or too few measured")
} else {
    TooManyTooFew <- SHmeasures %>% filter(NumLive != NumMeasured) %>% 
...
if(EstuaryCode %in% "SL" | EstuaryCode %in% "LX" | EstuaryCode %in% "LW"){
  ...
  SHMismatch <- union(Over10Measured, union(Over50Measured, TooManyTooFew)) %>% arrange(QuadratID)
}else{
...
`r if (nrow(ERR_Table3a) > 0 & EstuaryCode %in% "SL" | EstuaryCode %in% "LX" | EstuaryCode %in% "LW") {kable(ERR_Table3a, caption = "Childless Primary Keys. SampleEvents must have 15 child records unless a valid comment is present.")}`
`r if (nrow(ERR_Table3a) > 0 & EstuaryCode %in% "TB") {kable(ERR_Table3a, caption = "Childless Primary Keys. SampleEvents must have 5 child records unless a valid comment is present.")}`
