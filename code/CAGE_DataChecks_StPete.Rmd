...
hsdbTripInfo <- hsdbTripInfo %>% 
  filter(substring(TripID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & grepl("Cage", TripType))  %>%
  arrange(TripID)

TripInfo1 <- dboTripInfo %>% 
  filter(substring(TripID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & grepl("Cage", TripType) & DataStatus == "Proofed") %>%
  arrange(TripID)
...
hsdbSampleEvent <- hsdbSampleEvent %>% 
  ...
  filter(substring(SampleEventID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(SampleEventID,3,6) == "CAGE") %>%
  arrange(TripDate)

SampleEvent1 <- dboSampleEvent %>% 
  mutate(TripDate = as.Date(substring(SampleEventID, 8, 15), format = "%Y%m%d"),
         FixedLocationID = substring(SampleEventID, 19, 22)) %>%
  filter(substring(SampleEventID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(SampleEventID,3,6) == "CAGE" & DataStatus == "Proofed") %>%
...
hsdbSampleEventWQ <- hsdbSampleEventWQ %>% 
  ...
  filter(substring(SampleEventWQID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(SampleEventWQID,3,6) == "CAGE") %>%
  arrange(TripDate)

SampleEventWQ1 <- dboSampleEventWQ %>% 
  mutate(TripDate = as.Date(substring(SampleEventWQID, 8, 15), format = "%Y%m%d"),
         FixedLocationID = substring(SampleEventWQID, 19, 22)) %>%
  filter(substring(SampleEventWQID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(SampleEventWQID,3,6) == "CAGE" & DataStatus == "Proofed") %>%
...
Counts1 <- dboCageCount %>% 
  ...
  filter(substring(SampleEventID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & DataStatus == "Proofed") %>%
...
hsdbSH <- hsdbCageSH %>% 
  ...
  filter(substring(CageCountID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(CageCountID,3,6) == "CAGE") %>%
  arrange(TripDate)
...
SH1 <- dboCageSH %>%
  mutate(TripDate = as.Date(substring(CageCountID, 8, 15), format = "%Y%m%d"),
    ...
    filter(substring(CageCountID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & DataStatus == "Proofed") %>%
  left_join(FixedLocations1, Counts1, by = c("FixedLocationID")) %>%
