...
null_SurveyQuads <- filter(SurveyQuadrat1, is.na(NumLive) | is.na(NumDead) | is.na(NumLegal) | is.na(TotalWeight) | is.na(NumDrills) & !is.na(Comments)) %>%
  ...
  # Out of range values
SurveyQuad_range <- filter(SurveyQuadrat1, NumLive < 0 | NumLive > 500 | NumDead < 0 | NumDead > 300 | NumLegal < 0 | TotalWeight < 0.1 | TotalWeight > 10.00 | NumDrills < 0 | NumDrills > 5) %>%
  select(QuadratID, NumLive, NumDead, NumLegal, TotalWeight, NumDrills, Comments) 
...
LegalMeasures <- SurveySH1 %>% # Counts up the number of Legal measurements that were made
  group_by(QuadratID) %>%
  summarise(count_legals = sum(!is.na(ShellHeight) & ShellHeight > 74)) %>% #Sums legal-sized ShellHeight per QuadratID
...
`r if (nrow(LegalMeasures) > 0) {kable(LegalMeasures, caption = "Mismatched number legal (> 74 mm SH) measured and reported as NumLegal. NumLegal must equal count_Legals, unless: NumLive is greater than 50. Comment required for mismatched values.")}`
