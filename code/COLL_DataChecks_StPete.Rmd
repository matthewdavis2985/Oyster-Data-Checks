...
CI_range2 <- CI1 %>%
  mutate(
    ...
    ErrorCode = case_when(
      ...
      TRUE ~ ErrorCode),
    ErrorCode = case_when(
      ShellWetWeight < (TotalWeight * 0.65) | ShellWetWeight > (TotalWeight * 0.99) ~ paste(ErrorCode, "D"),
      TRUE ~ ErrorCode),
    ErrorCode = case_when(
...
Dermo_range <- Dermo1 %>%
  mutate(
    ...
    ErrorCode = case_when(
      ...
      TRUE ~ ErrorCode),
    ErrorCode = case_when(
      ShellWetWeight < (TotalWeight * 0.65) | ShellWetWeight > (TotalWeight * 0.99) ~ paste(ErrorCode, "E"),
      TRUE ~ ErrorCode),
    ErrorCode = case_when(
...
Dermo_rangecaption <- "Out of range Dermo data. Comment required for out of range values. <br>(A)Expected range for ShellHeight is 10 to 150 mm. <br>(D)Expected range for TotalWeight is 10 to 250 g. <br>(E)Expected range for ShellWetWeight is 65% to 99% of TotalWeight.  <br>(F)Expected ranges for DermoMantle and DermoGill is 0 to 5."
