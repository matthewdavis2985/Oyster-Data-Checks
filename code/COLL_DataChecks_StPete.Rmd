...
Database = "Oysters_24-05-31_test"
...
#Get all Repro data to check data status, then limit to proofed data for datachecks
Repro_t <- dboRepro %>% 
  mutate(TripDate = as.Date(substring(SampleEventID, 8, 15), format = "%Y%m%d"),
         FixedLocationID = substring(SampleEventID, 19, 22)) %>% 
  filter(substring(SampleEventID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & substring(SampleEventID,3,6) == "COLL" & DataStatus == "Proofed") %>%
  ...
  arrange(TripDate, StationNumber, OysterID)
Repro1 <- Repro_t %>% filter(DataStatus == "Proofed")
...
ERR_Table3 <- distinct(ERR_Table3) %>% mutate(Table = substring(Table, 13)) %>% table() %>% as.data.frame() %>% pivot_wider(id_cols = ChildlessPrimaryKey, names_from = Table, values_from = Freq)
