...
# Set your variables
CheckStart <- as.Date("2024-06-01")  # Start Date for Checks
CheckEnd <- as.Date("2024-06-30")  # End Date for Checks
DataManager = "Erica Levine"
Database = "Oysters_24-08-01"
...
if (nrow(Repro1[duplicated(Repro1$OysterID), ]) > 0) {
  ERR_Table1f <- bind_rows(ERR_Table1, data.frame(schema = "dbo", Table = "Repro", ReproSH1[duplicated(Repro1$OysterID), ])) %>% 
    mutate(PrimaryKey = OysterID) %>%
    select(schema, Table, PrimaryKey)
}
