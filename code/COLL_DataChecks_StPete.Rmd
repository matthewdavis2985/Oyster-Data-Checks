...
CheckEnd <- as.Date("2024-05-31")  # End Date for Checks
EstuaryCode = "SL"  # Estuary Code. Use 2-letter code in Primary Keys
DataManager = "Erica Levine"
Database = "Oysters_24 06 27"
...
CI_range2 <- CI1 %>%
  ...
  select(OysterID, TotalWt, TarePanWt, TissueWetWt, ShellWetWt, TissueDryWt, ShellDryWt, ErrorCode, Comments)
...
Dermo_rangecaption <- "Out of range Dermo data. Comment required for out of range values. <br>(A)Expected range for ShellHeight is 10 to 150 mm. <br>(D)Expected range for TotalWeight is 10 to 250 g. <br>(E)Expected range for ShellWetWeight is 60% to 99% of TotalWeight.  <br>(F)Expected ranges for DermoMantle and DermoGill is 0 to 5."
...
`r if (nrow(ERR_Table3) > 0) {kable(ERR_Table3, caption = "Childless Primary Keys. Primary Keys MUST have at least one child record! SampleEvents must have 15 child records unless a valid comment is present. *Primary keys will be Childless until data is proofed.*")}`
...
EXECUTE [dbo].[spChecksCollections]
