...
# Set your variables
CheckStart <- as.Date("2023-03-01")  # Start Date for Checks
CheckEnd <- as.Date("2023-03-31")  # End Date for Checks
EstuaryCode = "SL"  # Estuary Code. Use 2-letter code in Primary Keys
DataManager = "Erica Levine"
Database = "Oysters_24-06-21"
...
ERR_Table3 <- if(length(ERR_Table3) > 0){distinct(ERR_Table3) %>% mutate(Table = substring(Table, 13)) %>% table() %>% as.data.frame() %>% pivot_wider(id_cols = ChildlessPrimaryKey, names_from = Table, values_from = Freq)} else {data.frame()}
