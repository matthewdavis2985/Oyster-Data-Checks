...
Rcrt_dates <- Recruitment1  %>% mutate(SampleEvent = substr(SampleEventID, 1, 17)) %>% dplyr::select(SampleEvent, FixedLocationID, Estuary, StationNumber, DeployedDate, NumDays, Comments) %>% unique() %>% arrange(SampleEvent) %>% group_by(SampleEvent, Estuary, DeployedDate, NumDays) %>% mutate(FLIDs = paste0(FixedLocationID, collapse = ", "), Stations = paste0(StationNumber, collapse = ", "), Comments_all = paste0(Comments, collapse = " :: ")) %>% dplyr::select(-c(FixedLocationID, StationNumber, Comments)) %>% unique()
