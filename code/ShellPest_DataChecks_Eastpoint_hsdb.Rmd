...
sorc <- list(ShellPest1$OysterID, ShellPest1$SampleEventID)
dest <- list(hsdbCI$OysterID, hsdbCI$SampleEventID)
tabl <- list("OysterID", "SampleEvent")
...
for (i in seq_along(sorc)) {
  ...
  # Check if the id exists in hsdbCI$OysterID
    ...
    # Add id to ERR_Table if it is missing
      ERR_Table2 <- rbind(ERR_Table2, data.frame(Key = tabl[[i]], OrphanForeignKey = check_id))
    }
  }
}
