...
if (nrow(CI1[duplicated(CI1$OysterID), ]) > 0) {
  ERR_Table1d <- bind_rows(ERR_Table1, data.frame(schema = "hsdb", Table = "ConditionIndex", CI1[duplicated(CI1$OysterID), ])) %>% 
    mutate(PrimaryKey = OysterID) %>%
    select(schema, Table, PrimaryKey)
}

if (nrow(Dermo1[duplicated(Dermo1$OysterID), ]) > 0) {
  ERR_Table1e <- bind_rows(ERR_Table1, data.frame(schema = "hsdb", Table = "Dermo", Dermo1[duplicated(Dermo1$OysterID), ])) %>% 
    mutate(PrimaryKey = OysterID) %>%
    select(schema, Table, PrimaryKey)
}

if (nrow(Repro1[duplicated(Repro1$OysterID), ]) > 0) {
  ERR_Table1f <- bind_rows(ERR_Table1, data.frame(schema = "hsdb", Table = "Repro", Repro1[duplicated(Repro1$OysterID), ])) %>% 
    mutate(PrimaryKey = OysterID) %>%
    select(schema, Table, PrimaryKey)
}

ERR_Table1 <- rbind(ERR_Table1a, ERR_Table1b, ERR_Table1c, ERR_Table1d, ERR_Table1e, ERR_Table1f)
rm(ERR_Table1a, ERR_Table1b, ERR_Table1c, ERR_Table1d, ERR_Table1e, ERR_Table1f)
...
ConditionIndex_nulls <- filter(CI1, is.na(OysterID) | is.na(SampleEventID)) %>%
  select(OysterID, SampleEventID, Comments)

Dermo_nulls <- filter(Dermo1, is.na(OysterID) | is.na(SampleEventID)) %>%
  select(OysterID, SampleEventID, Comments)

Repro_nulls <- filter(Repro1, is.na(OysterID) | is.na(SampleEventID)) %>%
  select(OysterID, SampleEventID, Comments)

# Remove unneeded data frames and values

rm(dest, FixedLocations1, hsdbRepro, hsdbDermo, hsdbCI, hsdbSampleEvent, hsdbSampleEventWQ, hsdbTripInfo, fkid, pkid, numb, sorc, tabl, check_id, i, j)
...
for(i in list(EstuaryCode)) {
  checks <- NULL
  checks[i] <- paste0("EXECUTE [hsdb].[spChecksCollections] @CheckStart = &apos;", CheckStart, "&apos;, @CheckEnd = &apos;", CheckEnd, "&apos;, @EstuaryCode = &apos;", i, "&apos;, @DataManager = &apos;", DataManager, "&apos;;")
  }
```
