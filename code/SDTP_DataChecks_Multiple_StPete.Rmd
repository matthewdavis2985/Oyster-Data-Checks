...
Dry_ranges <- filter(SedimentTrap1, FilterDryWeight < FilterTareWeight | FilterDryWeight > 12.5 | PanDryWeight < PanTareWeight | PanDryWeight > 600 | CrucibleDW < TareCrucible | AshWeight < TareCrucible | CrucibleDW < AshWeight) %>%
...
if(nrow(Dry_ranges) > 0) {
  Dry_ranges <- Dry_ranges %>% 
    mutate(Error = case_when(FilterDryWeight < FilterTareWeight ~ "Small Filter DW",
                             ...
                             CrucibleDW < AshWeight ~ "Crucible DW is less than AW"))
}
