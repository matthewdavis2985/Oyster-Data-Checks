...
SampleEvent_nulls <- filter(SampleEvent1, is.na(SampleEventID) | is.na(TripID) ) %>%
...
SurveySH_nulls <- filter(SurveySH1, is.na(QuadratID) | is.na(QuadratID) ) %>%
...
EXEC sp_CompleteSurvey_hsdb \'`r EstuaryCode`\', \'`r CheckStart`\',
...
INSERT INTO #ValidTrips (TripID) SELECT hsdb.TripID FROM TripInfo WHERE
...
UPDATE hsdb.SurveyQuadrat SET DataStatus = \'Completed\', CompletedBy =
\'`r DataManager`\', DateCompleted = \'`r format(Sys.Date(), "%d-%B-%Y")`\'
WHERE SampleEventID IN (SELECT SampleEventID FROM SampleEvent WHERE
TripID IN (SELECT TripID FROM #ValidTrips));

UPDATE hsdb.SurveySH SET DataStatus = \'Completed\', CompletedBy =
\'`r DataManager`\', DateCompleted = \'`r format(Sys.Date(), "%d-%B-%Y")`\'
