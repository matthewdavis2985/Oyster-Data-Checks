...
EstuaryCode = "PE"  # Estuary Code. Use 2-letter code in Primary Keys
DataManager = "Matthew Davis"
Database = "OysterLocalMD20240205"
...
null_SedTraps2 <- filter(SedimentTrap1, is.na(FilterTareWeight) & is.na(FilterDryWeight) & !is.na(Comments)) %>%
...
Days_range <- filter(SedimentTrap1, NumDays < 21 | NumDays > 36) %>%
  select(TripDate, DeployedDate, NumDays, StationNumber) %>%
  distinct()
...
Tare_ranges <- filter(SedimentTrap1, FilterTareWeight < 0 | FilterTareWeight > 5.00 | PanTareWeight < 0 | PanTareWeight > 600.00) %>%
  select(TripDate, DeployedDate, StationNumber, CupSampleID, FilterTareWeight, PanTareWeight, Comments)
Dry_ranges <- filter(SedimentTrap1, FilterDryWeight < FilterTareWeight | FilterDryWeight > 20 | PanDryWeight < PanTareWeight | PanDryWeight > 900) %>%
...
`r if (nrow(Tare_ranges) > 0) {kable(Tare_ranges, caption = "Out of range Tare data. Expected range for filters is 0 to 5.00 grams. Expected range for pans is 0 to 600.00 grams. Comment suggested for out of range values.")}`

`r if (nrow(Dry_ranges) > 0) {kable(Dry_ranges, caption = "Out of range Dry data. Expected range for dry filters is FilterTare to 20.00 grams. Expected range for dry pans is PanTare to 900.00 grams. Comment suggested for out of range values.")}`

`r if (nrow(Biota_ranges) > 0) {kable(Biota_ranges, caption = "Out of range Biota data. Expected range for Drills is 0 to 5. Expected range for Crabs is 0 to 2. Expected range for Hermit Crabs is 0 to 20. Expected range for Fish is 0 to 2. Expected range for OtherBiota is 0 to 1. Comment suggested for out of range values.")}`
