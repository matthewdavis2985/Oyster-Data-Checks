...
Counts2 <- Counts1 %>% mutate(TripID = substr(SampleEventID, 1, 17)) %>% 
  dplyr::select(TripID, DeployedDate) %>% unique()
...
`r kable(left_join(TripInfo2, Counts2) %>% dplyr::select(TripID:TripDate, DeployedDate, everything()), caption = "Data from these trips is included in these checks")`
