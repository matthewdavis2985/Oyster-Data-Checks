...
TripInfo1 <- hsdbTripInfo %>% 
  filter(substring(TripID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate < CheckEnd & TripType == "Sediment Trap" & DataStatus == "Proofed") %>%
  arrange(TripID)
...
SampleEvent1 <- hsdbSampleEvent %>% 
  ...
  filter(substring(SampleEventID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate < CheckEnd & substring(SampleEventID,3,6) == "SDTP" & DataStatus == "Proofed") %>%
...
SampleEventWQ1 <- hsdbSampleEventWQ %>% 
  ...
  filter(substring(SampleEventWQID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate < CheckEnd & substring(SampleEventWQID,3,6) == "SDTP" & DataStatus == "Proofed") %>%
  left_join(FixedLocations1, SampleEventWQ1, by = c("FixedLocationID")) %>%
  arrange(TripDate, StationNumber, SampleEventWQID)

SedimentTrap1 <- hsdbSedimentTrap %>% 
  mutate(TripDate = as.Date(substring(SampleEventID, 8, 15), format = "%Y%m%d"),
         FixedLocationID = substring(SampleEventID, 19, 22),
...
#   filter(substring(TripID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate < CheckEnd & TripType == "Sediment Trap")  %>%
#   arrange(TripID)
# 
# hsdbSampleEvent <- hsdbSampleEvent %>% 
#   mutate(TripDate = as.Date(substring(SampleEventID, 8, 15), format = "%Y%m%d")) %>%
#   filter(substring(SampleEventID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate < CheckEnd & substring(SampleEventID,3,6) == "SDTP") %>%
#   arrange(TripDate)
# 
# hsdbSampleEventWQ <- hsdbSampleEventWQ %>% 
#   mutate(TripDate = as.Date(substring(SampleEventWQID, 8, 15), format = "%Y%m%d")) %>%
#   filter(substring(SampleEventWQID,1,2) == EstuaryCode & TripDate >= CheckStart & TripDate < CheckEnd & substring(SampleEventWQID,3,6) == "SDTP") %>%
#   arrange(TripDate)
# 
...
if (nrow(SedimentTrap1[duplicated(SedimentTrap1$CupSampleID), ]) > 0) {
  ERR_Table1d <- bind_rows(ERR_Table1, data.frame(schema = "hsdb", Table = "SedimentTrap", SedimentTrap1[duplicated(SedimentTrap1$CupSampleID), ])) %>% 
...
ERR_Table1 <- rbind(ERR_Table1a, ERR_Table1b, ERR_Table1c, ERR_Table1d)
rm(ERR_Table1a, ERR_Table1b, ERR_Table1c, ERR_Table1d)
...
rm(dboFixedLocations, dest, FixedLocations1, hsdbSedimentTrap, hsdbSampleEvent, hsdbSampleEventWQ, hsdbTripInfo, fkid, pkid, numb, sorc, tabl, check_id, i, j)

# Set Error Messages for Database Errors

if (nrow(ERR_Table1) > 0 | nrow(ERR_Table2) > 0 | nrow(ERR_Table3) > 0 | nrow(ERR_Table4 > 0) | nrow(ERR_Table4a > 0)) {
  MessERR_Tables1 = "STOP! These are critical errors that must be fixed before proceeding!"
  MessERR_Tables2 = ""
} else {
  MessERR_Tables1 = ""
  MessERR_Tables2 = "Congratulations! No Database errors found. Primary Keys are unique and not childless. Foreign Keys are not orphans and are properly formed. You may proceed."
}
```
...
null_SedTraps <- filter(SedimentTrap1, is.na(FilterTareWeight) & is.na(FilterDryWeight) & is.na(NumDrills) & is.na(NumCrabs) & is.na(NumHermitCrabs) & is.na(NumFish) & is.na(NumOtherBiota) & !is.na(Comments)) %>%
  select(TripDate, DeployedDate, StationNumber, CupSampleID, FilterTareWeight, FilterDryWeight, NumDrills, NumCrabs, NumHermitCrabs, NumFish, NumOtherBiota, Comments)
...
Tare_ranges <- filter(SedimentTrap1, FilterTareWeight < 0 | FilterTareWeight > 2.00 | PanTareWeight < 0 | PanTareWeight > 400.00) %>%
  select(TripDate, DeployedDate, StationNumber, CupSampleID, FilterTareWeight, PanTareWeight, Comments)
Dry_ranges <- filter(SedimentTrap1, FilterDryWeight < FilterTareWeight | FilterDryWeight > 10 | PanDryWeight < PanTareWeight | PanDryWeight > 600) %>%
  select(TripDate, DeployedDate, StationNumber, CupSampleID, FilterTareWeight, FilterDryWeight, PanTareWeight, PanDryWeight, Comments)
Biota_ranges <- filter(SedimentTrap1, NumDrills < 0 | NumDrills > 5 | NumCrabs < 0 | NumCrabs > 2 | NumHermitCrabs < 0 | NumHermitCrabs > 20 | NumFish < 0 | NumFish > 2 | NumOtherBiota < 0 | NumOtherBiota > 1) %>%
...
`r if (nrow(null_SedTraps) > 0) {kable(null_SedTraps, caption = "Null SedimentTrap Data. Comment Required for all Null values")}`
...
`r if (nrow(Tare_ranges) > 0) {kable(Tare_ranges, caption = "Out of range Tare data. Expected range for filters is 0 to 2.00 grams. Expected range for pans is 0 to 400.00 grams. Comment required for out of range values.")}`

`r if (nrow(Dry_ranges) > 0) {kable(Dry_ranges, caption = "Out of range Dry data. Expected range for dry filters is FilterTare to 10.00 grams. Expected range for dry pans is PanTare to 600.00 grams. Comment required for out of range values.")}`

`r if (nrow(Biota_ranges) > 0) {kable(Biota_ranges, caption = "Out of range Biota data. Expected range for Drills is 0 to 5. Expected range for Crabs is 0 to 2. Expected range for Hermit Crabs is 0 to 20. Expected range for Fish is 0 to 2. Expected range for OtherBiota is 0 to 1. Comment required for out of range values.")}`
...
EXEC sp_CompleteSedimentTrap '`r EstuaryCode`', '`r CheckStart`',
...
DataStatus = 'Proofed' AND TripID like '`r EstuaryCode`SDTP%';

UPDATE TripInfo SET DataStatus = 'Completed', CompletedBy =
'`r DataManager`', DateCompleted = '`r format(Sys.Date(), "%Y-%m-%d")`'
...
UPDATE SedimentTrap SET DataStatus = 'Completed', CompletedBy =
'`r DataManager`', DateCompleted = '`r format(Sys.Date(), "%%Y-%m-%d")`'
WHERE SampleEventID IN (SELECT SampleEventID FROM SampleEvent WHERE
TripID IN (SELECT TripID FROM #ValidTrips));
...
INSERT INTO hsdb.SedimentTrap SELECT \* FROM SedimentTrap WHERE DataStatus
= 'Completed';
