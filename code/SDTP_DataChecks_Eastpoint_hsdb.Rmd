...
if (nrow(Temp_range) > 0 | nrow(Sal_range) > 0 | nrow(DO_range) > 0 | nrow(pH_range) > 0 | nrow(Depth_range) > 0 | nrow(SampleDepth_range) > 0 | nrow(Secchi_range) > 0 | nrow(TurbidityYSI_range) > 0) {
  MessWQ_ranges = ""
} else {
...
EXEC sp_CompleteSedimentTrap_hsdb \'`r EstuaryCode`\', \'`r CheckStart`\',
...
UPDATE hsdb.SedimentTrap SET DataStatus = \'Completed\', CompletedBy =
\'`r DataManager`\', DateCompleted = \'`r format(Sys.Date(), "%%Y-%m-%d")`\'
WHERE SampleEventID IN (SELECT SampleEventID FROM SampleEvent WHERE
TripID IN (SELECT TripID FROM #ValidTrips));
