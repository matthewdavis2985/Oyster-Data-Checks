...
# Set your variables
CheckStart <- as.Date("2026-01-01")  # Start Date for Checks
CheckEnd <- as.Date("2026-03-28")  # End Date for Checks
DataManager = "Erica Williams"
Database = "Oysters_26-04-29"
Server = "localhost\\ERICALOCALSQL"

EstuaryCode = c(params$CERP)  #params$[project] or use 2-letter code in Primary Keys for single estuary
...
if (nrow(ERR_Table1) > 0 | nrow(ERR_Table2) > 0 | nrow(ERR_Table3) > 0 | nrow(ERR_Table4) > 0 | nrow(ERR_Table4a) > 0 | nrow(ERR_Table4b) > 0 | nrow(ERR_Table4c) > 0 | nrow(TripInfo_nulls) > 0 | nrow(SampleEvent_nulls) > 0 | nrow(SampleEventWQ_nulls) > 0 | nrow(ConditionIndex_nulls) > 0 | nrow(Dermo_nulls) > 0 | nrow(Repro_nulls) > 0) {
  MessERR_Tables1 = "STOP! These are potential critical errors that must be checked before proceeding!"
  MessERR_Tables2 = ""
} else {
  MessERR_Tables1 = ""
