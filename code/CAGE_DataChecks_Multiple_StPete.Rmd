...
`r kable(Counts1 %>% mutate(CageCountID = sub("_(D|R)_", "_", CageCountID)) %>% dplyr::select(CageCountID, CageColor, DataType, TotalCount) %>% pivot_wider(names_from = DataType, values_from = TotalCount) %>% mutate(CageCountID = paste0(substr(CageCountID,1,2), " ", substr(CageCountID, 19, 22))), caption = "Cage counts")`
