...
hsdbTripInfo <- hsdbTripInfo %>% 
  filter(substring(TripID,1,2) %in% EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & grepl("Cage", TripType))  %>%
  arrange(TripID)

TripInfo1 <- dboTripInfo %>% 
  filter(substring(TripID,1,2) %in% EstuaryCode & TripDate >= CheckStart & TripDate <= CheckEnd & grepl("Cage", TripType) & DataStatus == "Proofed") %>%
  arrange(TripID)
...
for the site(s) <p> `r EstuaryCode` </p2>\
