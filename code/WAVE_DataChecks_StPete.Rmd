...
EXEC sp_CompleteWave \'`r EstuaryCode`\', \'`r CheckStart`\',
...
DataStatus = \'Proofed\' AND TripID like \'`r EstuaryCode`WAVE%\';

UPDATE TripInfo SET DataStatus = \'Completed\', CompletedBy =
\'`r DataManager`\', DateCompleted = \'`r ceiling_date(Sys.Date(), "month") - 1`\'
...
UPDATE Wave SET DataStatus = \'Completed\', CompletedBy =
\'`r DataManager`\', DateCompleted = \'`r ceiling_date(Sys.Date(), "month") - 1`\'
WHERE SampleEventID IN (SELECT SampleEventID FROM SampleEvent WHERE
TripID IN (SELECT TripID FROM #ValidTrips));
...
INSERT INTO hsdb.Wave SELECT \* FROM Wave WHERE DataStatus
= \'Completed\';
...
AND DataStatus = 'Proofed' AND TripID like '`r EstuaryCode`WAVE%';

-- Query the TripInfo table using the temporary table
UPDATE TripInfo 
SET DataStatus = 'Completed', CompletedBy = '`r DataManager`', DateCompleted = '`r ceiling_date(Sys.Date(), "month") - 1`'
WHERE TripID IN (SELECT TripID FROM #ValidTrips);
