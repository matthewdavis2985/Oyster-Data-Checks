...
null_Recs <- filter(Recruitment1, is.na(NumBottom) & !is.na(Comments) & (ShellPosition >= 2 & ShellPosition <= 5 | ShellPosition >= 8 & ShellPosition <= 11)) %>%
...
EXEC sp_CompleteRecruitment \'`r EstuaryCode`\', \'`r CheckStart`\',
...
DataStatus = \'Proofed\' AND TripID like \'`r EstuaryCode`RCRT%\';
...
UPDATE Recruitment SET DataStatus = \'Completed\', CompletedBy =
\'`r DataManager`\', DateCompleted = \'`r format(Sys.Date(), "%d-%B-%Y")`\'
WHERE SampleEventID IN (SELECT SampleEventID FROM SampleEvent WHERE
TripID IN (SELECT TripID FROM #ValidTrips));
...
INSERT INTO hsdb.Recruitment SELECT \* FROM Recruitment WHERE DataStatus
= \'Completed\';
...
AND DataStatus = 'Proofed' AND TripID like '`r EstuaryCode`RCRT%';
